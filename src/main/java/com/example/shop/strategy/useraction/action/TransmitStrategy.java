...
public class TransmitStrategy extends ActionStrategy {
    ...
    public boolean doAction(StringRedisTemplate redisTemplate, RedisScript<Void> setUserAction,
        ...
        Object cache = redisTemplate.opsForHash().get(RedisKeyConstant.REDIS_TRANSMIT_STATUS + userId, infoId);
        ...
        RLock rLock = redissonClient.getLock(REDIS_TRANSMIT_LOCK + infoId);
        if (rLock == null) {
        ...
        redisTemplate.execute(setUserAction, List.of(REDIS_TRANSMIT_STATUS + userId, REDIS_TRANSMIT_COUNT + infoId),
                infoId, status);
        log.info("Update transmit userId {} infoId {}, status {}, count {}!", userId, infoId, status,
                redisTemplate.opsForValue().get(RedisKeyConstant.REDIS_TRANSMIT_COUNT + infoId));
        rLock.unlock();
        return true;
    }
}
